...
Active skin is **Sora Machine** — an editorial night-sky palette: bone paper, night-navy ink, sun-shard amber accent, twilight-slate muted, reading-line blue for external links. It's designed to look good out of the box; swap these values (or run [`onboarding.md`](onboarding.md)) and every new diagram inherits the new skin without touching any type-specific logic.
...
|---|---|---|---|
| `paper` | Page background, default node fill | `#f2eee3` (bone) | `#161d30` (night-navy) |
| `paper-2` | Diagram container bg, secondary fill | `#e6e1d3` | `#20293f` |
| `ink` | Primary text, primary stroke | `#1b2033` (night-navy) | `#f2eee3` (bone) |
| `muted` | Secondary text, default arrow stroke | `#4a5570` (twilight-slate) | `#a9b2c6` (moon-grey) |
| `soft` | Sublabels, boundary labels | `#7d879e` | `#7d879e` |
| `rule` | Hairline borders | `rgba(27,32,51,0.12)` | `rgba(242,238,227,0.12)` |
| `rule-solid` | Stronger borders, baselines | `#c4bfb1` (dust) | `rgba(169,178,198,0.25)` |
| `accent` | Focal / 1–2 max per diagram | `#d4842a` (sun-shard amber) | `#f0ac52` |
| `accent-tint` | Fill for accent-bordered boxes | `rgba(212,132,42,0.10)` | `rgba(240,172,82,0.12)` |
| `link` | HTTP/API calls, external arrows | `#3f6ba8` | `#7fa6dc` |

> **Brand palette source:** Sora Machine maps to a five-color world palette — `night-navy #161d30` (夜空), `twilight-slate #4a5570` (薄明), `bone #f2eee3` (紙・記録), `sun-shard amber #d4842a` (太陽の欠片 / Aro), `reading-line blue #3f6ba8` (読み取り線). The `soft`, `rule`, and `rule-solid` tokens are derived from ink at opacity and a lighter slate, to cover roles the world palette doesn't name directly.
...
Any `rgba(27,32,51, X)` in light becomes `rgba(242,238,227, X)` in dark. Same opacities, RGB flipped. The accent gets a slight hue-shift brighter to read on dark paper.
...
|---|---|---|---|---|
| `title` | Instrument Serif → Shippori Mincho (JP) | 1.75rem | 400 | Page H1 |
| `node-name` | Geist → Zen Kaku Gothic New (JP) | 12px | 600 | Human-readable labels |
...
| `callout` | Instrument Serif *italic* → Shippori Mincho (JP) | 14px | 400 | Editorial asides only |

### Font stack

```html
<link href="https://fonts.googleapis.com/css2?family=Instrument+Serif:ital@0;1&family=Geist:wght@400;500;600&family=Geist+Mono:wght@400;500;600&family=Shippori+Mincho:wght@400;600&family=Zen+Kaku+Gothic+New:wght@400;500;700&display=swap" rel="stylesheet">
```
...
**Japanese fallback rule:** Geist, Geist Mono, and Instrument Serif carry no CJK glyphs. Every family declaration must name its JP companion *after* the Latin face, so Latin renders in the brand face and Japanese falls through: `'Geist', 'Zen Kaku Gothic New', system-ui, sans-serif` · `'Geist Mono', 'Zen Kaku Gothic New', ui-monospace, monospace` · `'Instrument Serif', 'Shippori Mincho', serif`. Never put the JP family first.
